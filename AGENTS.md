...
19. **Identity GitOps & Group Governance**:
    ...
    - **Multi-Email Binding (Zero Alumni Group)**: Graduated members and alumni do not use an `alumni` group. Personal `@gmail.com` accounts are bound under `secondary_emails` in `members.yaml` for POSIX UID preservation and Web Print. Heavy GPU compute (JupyterHub) strictly authorizes against `primary_email` to ensure automatic compute revocation upon graduation.
...
23. **JupyterHub Spawner Hook Standards & Working Directory Architecture**:
    ...
    - Pre-spawn hooks and authenticator authorization MUST strictly match against `primary_email` or POSIX `uid` (`(&(objectClass=posixAccount)(|(mail={identifier})(uid={identifier})))`), intentionally excluding `secondary_emails` to ensure GPU compute access is automatically revoked when institutional `@ait.asia` accounts are deactivated upon graduation.
