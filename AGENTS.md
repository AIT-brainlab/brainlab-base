...
26. **Operator Device Group Assignment & Lazy WireGuard Tunnel Activation**: Personal operator workstations authenticating via Google OIDC MUST be assigned to the `tier-operators` group (via user `auto_groups` or peer settings) to inherit unrestricted access under `SysAdmin-God-Mode`. NetBird client status displaying `0/N Connected` under `Lazy connection: true` is standard power-saving idle behavior; WireGuard peer-to-peer tunnels awaken instantaneously upon transmission of active network traffic.
