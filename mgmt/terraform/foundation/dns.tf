...
locals {
  brainlab_records = {
    ...
    "dlms_stg"          = { name = "dlms-stg.brain.cs.ait.ac.th.", ip = "192.41.170.39", ttl = 300 }
    "wildcard_dlms_stg" = { name = "*.dlms-stg.brain.cs.ait.ac.th.", ip = "192.41.170.39", ttl = 300 }
...
# DPI `base` Subdelegation (NS)
resource "google_dns_record_set" "dpi_base_ns" {
  name         = "base.dpi.ait.ac.th."
  managed_zone = google_dns_managed_zone.dpi_zone.name
  type         = "NS"
  ttl          = 300
  rrdatas = [
...
resource "google_dns_record_set" "obs_dpi_ns" {
  name         = "obs.dpi.ait.ac.th."
  managed_zone = google_dns_managed_zone.dpi_zone.name
  type         = "NS"
  ttl          = 300
  rrdatas = [
